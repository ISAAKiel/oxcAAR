...
#' @export
roxcalCalibratedDate<-function(name, bp, std, cal_curve, sigma_ranges, raw_probabilities){
...
format.roxcalCalibratedDate <- function(x, ...){
  ...
  out_str$uncal_str<-paste(sprintf("BP = %d, std = %d",x$bp,x$std),"\n",sep = "")
  out_str$one_sigma_str<-formatFullSigmaRange(x$sigma_ranges$one_sigma,"one sigma")
  out_str$two_sigma_str<-formatFullSigmaRange(x$sigma_ranges$two_sigma,"two sigma")
  out_str$three_sigma_str<-formatFullSigmaRange(x$sigma_ranges$three_sigma,"three sigma")
...
plotRoxcalDateSystemGraphics<-function(x, ...){
  ...
  plot(years, probability, main = x$name, type = "n",ylim=c(max_prob/7*-1,max_prob))
  mtext(formatFullSigmaRange(x$sigma_ranges$one_sigma,"one sigma"), 3, line=0,cex=0.6,adj=1)
  mtext(formatFullSigmaRange(x$sigma_ranges$two_sigma,"two sigma"), 3, line=1,cex=0.6,adj=1)
  mtext(formatFullSigmaRange(x$sigma_ranges$three_sigma,"three sigma"), 3, line=2,cex=0.6,adj=1)
  polygon(years, probability, col="lightgrey")
  if (length(x$sigma_ranges$one_sigma[,1])>0)
  {
    y_pos<-max_prob/24*-1
    arrow_length<-max_prob/8
    arrows(x$sigma_ranges$one_sigma[,1],y_pos,x$sigma_ranges$one_sigma[,2],y_pos,length(x$sigma_ranges$one_sigma),col="black",code=3,angle=90,length=arrow_length,lty=1,lwd=2)
    y_pos<-y_pos*2
    arrows(x$sigma_ranges$two_sigma[,1],y_pos,x$sigma_ranges$two_sigma[,2],y_pos,length(x$sigma_ranges$two_sigma),col="black",code=3,angle=90,length=arrow_length,lty=1,lwd=2)
    y_pos<-y_pos/2*3
    arrows(x$sigma_ranges$three_sigma[,1],y_pos,x$sigma_ranges$three_sigma[,2],y_pos,length(x$sigma_ranges$three_sigma),col="black",code=3,angle=90,length=arrow_length,lty=1,lwd=2)
  }
