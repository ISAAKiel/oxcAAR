...
testthat::with_mocked_bindings(
  {
    ...
    test_that("oxcalCalibrate produces an oxcAARCalibratedDatesList", {
      this_result <- oxcalCalibrate(5000, 25, "KIA-12345")
      expect_equal(class(this_result), c("list", "oxcAARCalibratedDatesList"))
    })

    test_that("oxcalCalibrate produces an oxcAARCalibratedDatesList with correct oxcalCalibratedDates", {
      this_result <- oxcalCalibrate(5000, 25, "KIA-12345")[[1]]

      expect_equal(class(this_result), "oxcAARCalibratedDate")
      expect_equal(names(this_result), c(
        "name", "type", "bp", "std", "cal_curve", "sigma_ranges",
        "raw_probabilities", "posterior_sigma_ranges", "posterior_probabilities"
      ))
      ...
      expect_true(
        this_result$sigma_ranges$one_sigma[1] >= min(this_result$raw_probabilities$dates) &&
  ...
  executeOxcalScript = function(...) "ox_output.js",
  .package = "oxcAAR"
)
